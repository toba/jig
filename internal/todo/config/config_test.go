...
func TestLoadAppliesDefaults(t *testing.T) {
	...
	// Write minimal config (missing default_type) in new .jig.yaml format
	minimalConfig := `todo:
    path: ".issues"
`
...
func TestFindConfig(t *testing.T) {
	...
	t.Run("finds and migrates legacy .todo.yml", func(t *testing.T) {
		...
		// Should return the new .jig.yaml path
		...
		if _, err := os.Stat(expectedPath); err != nil {
			t.Errorf(".jig.yaml was not created: %v", err)
		}
	...
	t.Run("prefers .jig.yaml over .todo.yml", func(t *testing.T) {
		tmpDir := t.TempDir()
		// Write both files
		...
		if found != newPath {
			t.Errorf("FindConfig() = %q, want %q (should prefer .jig.yaml)", found, newPath)
		}
	})
}

func TestLoadFromDirectory(t *testing.T) {
	t.Run("loads config from directory with .jig.yaml", func(t *testing.T) {
		tmpDir := t.TempDir()
		configPath := filepath.Join(tmpDir, ConfigFileName)
		configYAML := `todo:
    path: custom-issues
