...
import (
	...
	"github.com/toba/jig/internal/todo/refry"
)
...
var refryCmd = &cobra.Command{
	Use:   "refry",
	Short: "Convert an hmans/beans project to todo format",
	Long: `Converts a project using the original hmans/beans issue tracker
(.beans.yml config, .beans/ data directory) to todo format
(.jig.yaml config, .issues/ with bucketed subdirectories).
