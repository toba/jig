...
func TestBuildRoadmap(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			issues: []*issue.Issue{
				{ID: "m1", Type: "milestone", Title: "v1.0", Status: "todo", CreatedAt: &now},
				{ID: "t1", Type: "task", Title: "Orphan", Status: "todo"},
			},
			wantMilestones:       0,
			wantUnscheduledOther: 1,
		},
	}
...
func TestFirstParagraph(t *testing.T) {
	tests := []struct {
		...
		{"empty body", "", ""},
		{"single line", "This is a description.", "This is a description."},
		{"multiple paragraphs", "First paragraph.\n\nSecond paragraph.", "First paragraph."},
		{"multiline first paragraph", "Line one\nLine two\n\nSecond para.", "Line one Line two"},
		{"skips headers at start", "## Checklist\n- item one", "- item one"},
		{
			"truncates long text",
			"This is a very long paragraph that exceeds two hundred characters and needs to be truncated so it does not take up too much space in the roadmap output. Lorem ipsum dolor sit amet consectetur adipiscing elit.",
			"This is a very long paragraph that exceeds two hundred characters and needs to be truncated so it does not take up too much space in the roadmap output. Lorem ipsum dolor sit amet consectetur adipi...",
		},
	}
